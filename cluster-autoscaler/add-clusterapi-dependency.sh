...
CODEGENERATOR_VERSION=release-1.11
K8S_UTILS_POINTER_VERSION=6999998975a717e7f5fc2a7476497044cb111854
...
if ! [[ -d $(go env GOPATH)/src/k8s.io/utils ]]; then
  git clone git@github.com:kubernetes/utils  $(go env GOPATH)/src/k8s.io/utils
fi
cd $(go env GOPATH)/src/k8s.io/utils
git fetch
git checkout ${K8S_UTILS_POINTER_VERSION}
cd -
rsync -av  $(go env GOPATH)/src/k8s.io/utils/pointer vendor/k8s.io/utils/
...
sed -i 's#k8s.io/klog#github.com/golang/glog#' cloudprovider/clusterapi/* vendor/sigs.k8s.io/cluster-api/pkg/apis/cluster/common/*
sed -i 's/klog/glog/g' cloudprovider/clusterapi/* vendor/sigs.k8s.io/cluster-api/pkg/apis/cluster/common/*
